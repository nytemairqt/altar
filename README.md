...
Once X-Code launches, click HISE button -> "Edit Schema"

Set the config to RELEASE.
...
If it fails the first time, you might need to enable C++20:

Open Projucer, drag the `/altar/DspNetworks/Binaries/AutoGeneratedProject.jucer` file into Projucer.

Click Settings cogwheel, set C++ version to C++20.

Recompile in X-Code, if you do it in HISE again it will overwrite our C++20 change.
...
If it doesn't work, you probably need to enable C++20 for the exported binary as well:

Open the AutoGeneratedProject.jucer file here in Projucer and once again, set the C++ Language Standard to C++ 20. Save & Open in IDE.
