...
{
// Parameter list for click_impl::click_t ----------------------------------------------------------

template <int NV>
using Gain = parameter::plain<core::gain<NV>, 0>;
using TempoSync = parameter::empty;
template <int NV>
using click_t_plist = parameter::list<Gain<NV>, TempoSync>;
}

template <int NV>
using click_t_ = container::chain<click_t_parameters::click_t_plist<NV>, 
...
template <int NV> struct instance: public click_impl::click_t_<NV>
{
	...
	struct metadata
	{
		...
		SNEX_METADATA_ENCODED_PARAMETERS(36)
		{
            ...
            0x005C, 0x0001, 0x0000, 0x6554, 0x706D, 0x536F, 0x6E79, 0x0063, 
	...
	instance()
	{
		...
		auto& soft_bypass = this->getT(0);                   // click_impl::soft_bypass_t<NV>
		auto& modchain = this->getT(0).getT(0);              // click_impl::modchain_t<NV>
		auto& tempo_sync1 = this->getT(0).getT(0).getT(0);   // click_impl::tempo_sync1_t<NV>
		auto& converter = this->getT(0).getT(0).getT(1);     // click_impl::converter_t<NV>
		auto& oscillator = this->getT(0).getT(0).getT(2);    // click_impl::oscillator_t<NV>
		auto& peak_unscaled = this->getT(0).getT(0).getT(3); // click_impl::peak_unscaled_t<NV>
		auto& ahdsr1 = this->getT(0).getT(1);                // click_impl::ahdsr1_t<NV>
		auto& file_player = this->getT(0).getT(2);           // click_impl::file_player_t<NV>
		auto& gain = this->getT(0).getT(3);                  // core::gain<NV>
	...
	void setExternalData(const ExternalData& b, int index)
	{
		...
		this->getT(0).getT(0).getT(2).setExternalData(b, index); // click_impl::oscillator_t<NV>
		this->getT(0).getT(0).getT(3).setExternalData(b, index); // click_impl::peak_unscaled_t<NV>
		this->getT(0).getT(1).setExternalData(b, index);         // click_impl::ahdsr1_t<NV>
		this->getT(0).getT(2).setExternalData(b, index);         // click_impl::file_player_t<NV>
	}
};
}
