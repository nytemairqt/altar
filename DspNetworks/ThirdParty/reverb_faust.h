...
#include "src/reverb_faust.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct reverb_faustMetaData {
		SN_NODE_ID("reverb_faust");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using reverb_faust = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _reverb_faust, reverb_faustMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
