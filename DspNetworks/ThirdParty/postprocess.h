...
template <int NV> struct postprocess: public data::base
{
    SNEX_NODE(postprocess);        
    
    struct MetadataClass
    {
        SN_NODE_ID("postprocess");
    };
    ...
    {       
        ...
        // Processing Order
        {
            ...
            registerCallback<24>(processingOrder);
...
private:
    ...
    // EQ Filter states - simplified biquad implementation
    struct BiquadState
    {
    ...
    float processEQ(float input, int channel)
    {
        ...
        // Process through all 9 EQ filters
        for (int i = 0; i < 9; ++i)
        {
            output = eqStates[channel][i].process(output);
        }
    ...
    {
        // Update HPF (filters 0 and 1 for 24dB/oct)
