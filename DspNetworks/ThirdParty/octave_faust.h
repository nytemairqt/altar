...
#include "src/octave_faust.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct octave_faustMetaData {
		SN_NODE_ID("octave_faust");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using octave_faust = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _octave_faust, octave_faustMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
