...
template <int NV> struct reverb: public data::base
{
    ...
    void prepare(PrepareSpecs specs) 
    {
        ...
        // Initialize allpass channel offsets
        for (int i = 0; i < numAllpassFilters; ++i)
        {
            allpassChannelOffset[i] = (i * 3 + 5) % 15 + 5; // Deterministic offset
        }
        ...
        // Initialize damping filter coefficients
        updateDampingFilter();
    ...
    template <typename T> void process(T& data)
    {
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            for (auto ch : data)
            {
                ...
                if (channelIndex == 0) 
                    lastDiffusedL = diffusedSample;
                else
                    lastDiffusedR = diffusedSample;
...
private:
    ...
    // Biquad Filter State for damping
    struct BiquadState
    {
